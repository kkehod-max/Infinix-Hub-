...
-- UI Animation ตอนโหลด (แบบ PIG HUB)
task.spawn(function()
...
-- ==================== ฟังก์ชันพื้นฐานทั้งหมด (จาก god2.txt) ====================
...
Players.PlayerAdded:Connect(function(player)
    player.CharacterAdded:Connect(function(character)
        if highlightEnabled then playerHighlights[player] = Instance.new("Highlight", character) end
            ...
            for _, player in pairs(Players:GetPlayers()) do
                if player ~= LocalPlayer and player.Character then
                    if not playerHighlights[player] then
                        playerHighlights[player] = Instance.new("Highlight", player.Character)
                    end
                end
            end
        end
...
local FOVSlider = Tab:Slider({
    ...
    Callback = function(value) FOVRadius = tonumber(value) or 120 end
})
...
local MeleeToggle = Tab_mods:Toggle({
    Title = "Melee Aura",
    Desc = "WideFists",
    Default = false,
...
myConfig:Register("Fists Modifier", MeleeToggle)
        ...
        for _, player in pairs(Players:GetPlayers()) do
            if player ~= LocalPlayer then
                if state and player.Character then
                    if not playerHighlights[player] then
                        playerHighlights[player] = Instance.new("Highlight", player.Character)
                        playerHighlights[player].FillColor = Color3.fromRGB(0, 170, 255)
                        playerHighlights[player].OutlineColor = Color3.fromRGB(0, 170, 255)
                    end
...
local JumpPowerToggle = Tab_Character:Toggle({
    ...
    Default = false,
    Callback = function(state) FlyEnabled = state; if not FlyEnabled then flying = false end end
})
myConfig:Register("JumpPower", JumpPowerToggle)

-- Anti Lock Toggle (ใหม่)
...
local AntiLockToggle2 = Tab_Character:Toggle({
    Title = "Anti Lock (Sky)",
    Default = false,
    Callback = function(state) getgenv().Sky = state; if state then getgenv().SkyAmount = 1500 end end
})
myConfig:Register("AntiLock", AntiLockToggle2)
...
local AntiKillToggle = Tab_Character:Toggle({
    ...
    Callback = function(state)
        enabled = state
        if state then if WindUI then WindUI:Notify({ Title = "Anti Kill Enabled", Duration = 3 }) end
        else if WindUI then WindUI:Notify({ Title = "❌ Anti Kill Disabled", Duration = 3 }) end end
    end
})
...
local function FindServer()
    ...
    for i = 1, 5 do
        local success, result = pcall(function()
            return HttpService:JSONDecode(game:HttpGet(string.format("https://games.roblox.com/v1/games/%d/servers/Public?sortOrder=Desc&limit=100%s", placeId, cursor and "&cursor=" .. cursor or "")))
        end)
        if success and result and result.data then
            for _, server in ipairs(result.data) do
                if server.playing < server.maxPlayers and server.playing <= 20 then table.insert(servers, server) end
            end
...
local Serverhop = Tab_misc:Button({
    ...
    Callback = function()
        ...
        if not success or not servers or not servers.data then
            warn("ไม่สามารถดึงข้อมูลเซิร์ฟเวอร์")
            return
        end
        local availableServers = {}
        for _, server in ipairs(servers.data) do
            if server.playing < server.maxPlayers and server.id ~= game.JobId then table.insert(availableServers, server) end
        end
        if #availableServers == 0 then
            warn("ไม่มีเซิร์ฟเวอร์ว่าง")
            return
        end
        table.sort(availableServers, function(a, b) return a.playing > b.playing end)
        local targetServer = availableServers[1]
        game.StarterGui:SetCore("SendNotification", { Title = "Server Hop", Text = "กำลังย้ายไปเซิร์ฟเวอร์...", Duration = 3 })
